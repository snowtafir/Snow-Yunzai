...
async function relpyPrivate (userId, msg) {
  ...
  let friend = Bot[this.e.self_id].fl.get(userId)
  if (friend) {
    logger.mark(`发送好友消息[${friend.nickname}](${userId})`)
    return await Bot[this.e.self_id].pickUser(userId).sendMsg(msg).catch((err) => {
      logger.mark(err)
    })
  }
}
...
async function makeForwardMsg (e, msg = [], dec = '') {
  let nickname = Bot[e.self_id].nickname
  if (e.isGroup) {
    let info = await Bot[e.self_id].getGroupMemberInfo(e.group_id, e.self_id)
