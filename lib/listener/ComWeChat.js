...
async function connectBot(data) {
  ...
  Bot[data.self_id] = {
    ...
    getFriendInfo: user_id => Bot[data.self_id].pickFriend(user_id).getInfo(),

    pickMember: (group_id, user_id) => {
      let i = { ...data, group_id, user_id }
      return {
        sendMsg: msg => sendFriendMsg(i, msg),
        recallMsg: () => false,
        makeForwardMsg: msg => makeFriendForwardMsg(i, msg),
        getInfo: () => getGroupMemberInfo(i, i.user_id),
        getAvatarUrl: async () => (await getGroupMemberInfo(i, i.user_id))["wx.avatar"],
      }
    },
