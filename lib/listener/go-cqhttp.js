...
function sendApi(ws, action, params) {
  let echo = randomUUID()
  let msg = JSON.stringify({ action, params, echo })
...
function getGroupInfo(data, group_id) {
  return data.sendApi("get_group_info", { group_id })
}

async function getGroupMemberList(data, group_id) {
  return (await data.sendApi("get_group_member_list", { group_id })).data
}

async function getGroupMemberMap(data, group_id) {
  let map = new Map()
  for (const i of (await getGroupMemberList(data, group_id)))
    map.set(i.user_id, i)
  return map
}
...
function getGuildInfo(data, guild_id) {
  return data.sendApi("get_guild_meta_by_guest", { guild_id })
}

async function getGuildChannelList(data, guild_id) {
  return (await data.sendApi("get_guild_channel_list", { guild_id })).data
}

async function getGuildChannelMap(data, guild_id) {
  let map = new Map()
  for (const i of (await getGuildChannelList(data, guild_id)))
    map.set(i.channel_id, i)
  return map
}

async function getGuildMemberList(data, guild_id, next_token) {
  return (await data.sendApi("get_guild_member_list", { guild_id, next_token })).data
}

async function getGuildMemberMap(data, guild_id) {
  let map = new Map()
  let next_token = ""
  while (true) {
    let list = await getGuildMemberList(data, guild_id, next_token)
  ...
  Bot[data.self_id] = {
    ...
    pickGroup: group_id => {
      if (typeof group_id == "string" && group_id.match("-")) {
        ...
        return {
          ...
          getMemberList: () => getGroupMemberList(i, i.group_id),
          getMemberMap: () => getGroupMemberMap(i, i.group_id),
    ...
    pickGuild: (guild_id, channel_id) => {
      ...
      return {
        ...
        recallMsg: message_id => recallMsg(i, message_id),
        makeForwardMsg: msg => makeGuildForwardMsg(i, msg),
        getInfo: () => getGuildInfo(i, i.guild_id),
        getChannelList: () => getGuildChannelList(i, i.guild_id),
        getChannelMap: () => getGuildChannelMap(i, i.guild_id),
        getMemberList: () => getGuildMemberList(i, i.guild_id),
        getMemberMap: () => getGuildMemberMap(i, i.guild_id),
        getMemberInfo: user_id => getGuildMemberInfo(i, user_id),
        pickMember: user_id => Bot[data.self_id].pickGuildMember(i.guild_id, i.channel_id, user_id),
      }
    },
