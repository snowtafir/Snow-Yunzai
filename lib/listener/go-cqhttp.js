...
function getGuildMemberInfo(data) {
...
async function connectBot(data) {
  Bot[data.self_id] = {
    ...
    pickGuildMember: (guild_id, channel_id, user_id) => {
      ...
      return {
        ...Bot[data.self_id].pickGuild(i.guild_id, i.channel_id),
        getInfo: () => getGuildMemberInfo(i),
        getAvatarUrl: async () => (await getGuildMemberInfo(i)).avatar_url,
      }
    },
    ...
    getGuildList: async () => (await data.sendApi("get_guild_list")).data,
    getGuildMap: () => Bot[data.self_id].tl,
  }
