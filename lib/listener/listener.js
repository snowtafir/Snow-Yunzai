...
function sendFriendMsg(data, msg) {
  ...
  return sendApi(data.ws, "send_msg", {
...
function sendGuildMsg(data, msg) {
  ...
  return sendApi(data.ws, "send_guild_channel_msg", {
...
function recallMsg(data, message_id) {
  ...
  return sendApi(data.ws, "delete_msg", {
    message_id,
  });
}
...
async function makeFriendForwardMsg(data, msg) {
  ...
  msg = await sendApi(data.ws, "send_private_forward_msg", {
    user_id: data.user_id,
...
async function makeGroupForwardMsg(data, msg) {
  ...
  msg = await sendApi(data.ws, "send_group_forward_msg", {
    group_id: data.group_id,
    messages: makeForwardMsg(data, msg),
  });
  msg.data = "群转发消息"
...
async function getGroupMemberInfo(data, user_id) {
  return (await sendApi(data.ws, "get_group_member_info", {
    group_id: data.group_id,
    user_id,
  })).data
}

async function getGuildMemberInfo(data, user_id) {
  return (await sendApi(data.ws, "get_guild_member_profile", {
    guild_id: data.guild_id,
    user_id,
  })).data
}
...
async function connectBot(data) {
  ...
  let login_info = await sendApi(data.ws, "get_login_info")
  let guild_info = await sendApi(data.ws, "get_guild_service_profile")

  let model = "TRSS-Yunzai"
  sendApi(data.ws, "_set_model_show", {
    model,
    model_show: model,
  })

  let fl = new Map()
  for (const i of (await sendApi(data.ws, "get_friend_list")).data)
    fl.set(i.user_id, i)

  let gl = new Map()
  for (const i of (await sendApi(data.ws, "get_group_list")).data)
    gl.set(i.group_id, i)

  let tl = new Map()
  for (const i of (await sendApi(data.ws, "get_guild_list")).data)
    tl.set(i.guild_id, i)

  Bot[data.self_id] = {
    ws: data.ws,
    sendApi: (action, params) => sendApi(data.ws, action, params),

    uin: login_info.user_id,
    nickname: login_info.nickname,
    stat: {
      start_time: data.time
    },

    tiny_id: guild_info.tiny_id,
    guild_nickname: guild_info.nickname,

    model,
    clients: (await sendApi(data.ws, "get_online_clients")).clients,
    version: (await sendApi(data.ws, "get_version_info")).data,
...
async function makeNotice(data) {
  switch (data.notice_type) {
    ...
    case "client_status":
      ...
      data.clients = (await sendApi(data.ws, "get_online_clients")).clients
