...
export default class gocqhttpAdapter {
  ...
  async getFriendMap(data) {
    const map = new Map()
    for (const i of (await this.getFriendList(data)))
      map.set(i.user_id, i)
    return map
  }
  ...
  async getGroupMap(data) {
    const map = new Map()
    for (const i of (await this.getGroupList(data)))
      map.set(i.group_id, i)

    for (const guild of (await this.getGuildList(data)))
      for (const channel of (await this.getGuildChannelList({ ...data, guild_id: guild.guild_id })))
        map.set(`${guild.guild_id}-${channel.channel_id}`, {
          ...guild,
          ...channel,
  ...
  async getGroupMemberMap(data) {
    const map = new Map()
    for (const i of (await this.getGroupMemberList(data)))
      map.set(i.user_id, i)
    return map
  }
  ...
  async getGuildChannelMap(data) {
    const map = new Map()
    for (const i of (await this.getGuildChannelList(data)))
      map.set(i.channel_id, i)
    return map
  }

  async getGuildMemberList(data, next_token) {
  ...
  async getGuildMemberMap(data) {
    ...
    let next_token = ""
    while (true) {
      const list = await this.getGuildMemberList(data, next_token)
