      ...
      switch (typeof data[i]) {
        ...
        case "object":
          if (Buffer.isBuffer(data[i]))
            data[i] = `base64://${Buffer.from(data[i], "utf8").toString("base64")}`;
...
global.segment = {
  ...
  face: function face(id) {
    return CQCode("face", { id });
  },

  record: function record(file) {
    return CQCode("record", { file });
  },

  video: function video(file) {
    return CQCode("video", { file });
  },

  at: function at(qq, name) {
    return CQCode("at", { qq, name });
  },

  share: function share(url, title, content, image) {
    return CQCode("share", { url, title, content, image });
  },

  music: function music(type, id, url, audio, title) {
    return CQCode("music", { type, id, url, audio, title });
  },

  image: function image(file, type, subType) {
    return CQCode("image", { file, type, subType });
  },

  reply: function reply(id, text, qq, time, seq) {
    return CQCode("reply", { id, text, qq, time, seq });
  },

  poke: function poke(qq) {
    return CQCode("poke", { qq });
  },

  gift: function poke(qq, id) {
    return CQCode("gift", { qq, id });
  },

  xml: function xml(data, resid) {
    return CQCode("xml", { data, resid });
  },

  json: function json(data, resid) {
    return CQCode("json", { data, resid });
  },

  cardimage: function cardimage(file, minwidth, minheight, maxwidth, maxheight, source, icon) {
    return CQCode("cardimage", { file, minwidth, minheight, maxwidth, maxheight, source, icon });
  },

  tts: function tts(text) {
    return CQCode("tts", { text });
  },
};
...
class PluginsLoader {
  ...
  packageTips(packageErr) {
    ...
    packageErr.forEach((v) => {
      ...
      logger.mark(
        `新增插件后请执行安装命令：${logger.red("pnpm install -P")} 安装依赖`
      );
      logger.mark(
        `如安装后仍未解决可联系插件作者将 ${logger.red(
          pack
        )} 依赖添加至插件的package.json dependencies中，或手工安装依赖`
      );
    });
