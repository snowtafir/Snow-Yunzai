...
global.segment = {
  CQCode: CQCode,
  face: id => CQCode("face", { id }),
  record: file => CQCode("record", { file }),
  video: file => CQCode("video", { file }),
  at: (qq, name) => CQCode("at", { qq, name }),
  share: (url, title, content, image) => CQCode("share", { url, title, content, image }),
  music: (type, id, url, audio, title) => CQCode("music", { type, id, url, audio, title }),
  image: (file, type, subType) => CQCode("image", { file, type, subType }),
  reply: (id, text, qq, time, seq) => CQCode("reply", { id, text, qq, time, seq }),
  poke: qq => CQCode("poke", { qq }),
  gift: (qq, id) => CQCode("gift", { qq, id }),
  xml: (data, resid) => CQCode("xml", { data, resid }),
  json: (data, resid) => CQCode("json", { data, resid }),
  cardimage: (file, minwidth, minheight, maxwidth, maxheight, source, icon) => CQCode("cardimage", { file, minwidth, minheight, maxwidth, maxheight, source, icon }),
  tts: text => CQCode("tts", { text }),
};
...
class PluginsLoader {
  ...
  dealMsg(e) {
    ...
    if (e.user_id && cfg.masterQQ.includes(Number(e.user_id) ? Number(e.user_id) : String(e.user_id))) {
      e.isMaster = true;
    }
