...
global.segment = {
  at: function at(qq, name) {
    return `[CQ:at,qq=${escape(qq)},name=${escape(name)}]`;
  },

  image: function image(file) {
    if (Buffer.isBuffer(file))
      file = `base64://${Buffer.from(file, "utf8").toString("base64")}`;
  ...
  video: function video(file) {
    if (Buffer.isBuffer(file))
      file = `base64://${Buffer.from(file, "utf8").toString("base64")}`;
    return `[CQ:video,file=${escape(file)}]`;
  },

  share: function share(url, title) {
    return `[CQ:share,url=${escape(url)},title=${escape(title)}]`;
  },

  music: function music(type, id) {
    return `[CQ:music,type=${escape(type)},id=${escape(id)}]`;
  },

  poke: function at(qq) {
    return `[CQ:poke,qq=${escape(qq)}]`;
  },

  xml: function xml(data, resid) {
    return `[CQ:xml,data=${escape(data)},resid=${escape(resid)}]`;
  },

  json: function json(data, resid) {
    return `[CQ:json,data=${escape(data)},resid=${escape(resid)}]`;
  },

  cardimage: function cardimage(file) {
    if (Buffer.isBuffer(file))
      file = `base64://${Buffer.from(file, "utf8").toString("base64")}`;
    return `[CQ:cardimage,file=${escape(file)}]`;
  },
